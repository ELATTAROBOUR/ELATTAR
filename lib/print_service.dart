...
class PrintService {
                      ...
                      child: pw.Center(
                        child: pw.FittedBox(
                          ...
                          child: pw.Column(
                            ...
                            children: [
                              pw.Text(
                                _cleanAndShapeText(ticket.customerName),
                                style: pw.TextStyle(
                                    fontSize: 12,
                                    fontWeight: pw.FontWeight.bold),
                              ...
                              pw.Text(
                                _cleanAndShapeText(ticket.customerPhone),
                                style: pw.TextStyle(
                                    fontSize: 9.5,
                                    fontWeight: pw.FontWeight.bold),
                                textAlign: pw.TextAlign.center,
                              ...
                              pw.SizedBox(height: 2),
                              pw.Text(
                                _cleanAndShapeText(ticket.problem),
                                style: pw.TextStyle(
                                    fontSize: 9.5,
                                    fontWeight: pw.FontWeight.bold),
                                textAlign: pw.TextAlign.center,
  ...
  static Future<Uint8List> generateReceiptPdf(Ticket ticket,
      {bool isDelivery = false,
      double? overrideCost,
      String? overridePaymentMethod,
      String? overridePaymentDetails}) async {
    final pdfTheme = await _getArabicTheme();
    final pdf = pw.Document();
    ...
    final rawPhone = (ticket.technicianPhone != null &&
            ...
            child: pw.Column(
              ...
              children: [
                ...
                if (ticket.expectedDelivery != null &&
                  ...
                  receiptRow('التسليم المتوقع', ticket.expectedDelivery!),
  ...
  static Future<void> printReceipt(Ticket ticket,
      {bool isDelivery = false,
      double? overrideCost,
      String? overridePaymentMethod,
  ...
  static pw.Widget _receiptRowHelper(String label, String value,
      {double fontSize = 14}) {
    final cleanLabel = _cleanAndShapeText(label);
    final cleanValue = _cleanAndShapeText(value);
    return pw.Padding(
  ...
  static Future<Uint8List> generateDailyClosurePdf(
      DateTime date, List<Ticket> tickets) async {
    final pdfTheme = await _getArabicTheme();
    final pdf = pw.Document();
      ...
      addRtl(
        pw.Padding(
          ...
          child: pw.Column(
            ...
            children: [
              pw.Text(
                _cleanAndShapeText('الفني: $techName (${list.length} أجهزة)'),
                style:
              ...
              pw.Padding(
                ...
                child: pw.Text(
                  _cleanAndShapeText('المجموع: ${techRevenue.toStringAsFixed(0)} | قطع: ${techParts.toStringAsFixed(0)} | عمولة: ${techEarn.toStringAsFixed(0)} | للمحل: ${storeShare.toStringAsFixed(0)}'),
    ...
    addRtl(_receiptRowHelper('نقدي (كاش)', '${cashSum.toStringAsFixed(0)} ج.م',
        fontSize: 7.5));
    addRtl(_receiptRowHelper(
        'فودافون كاش', '${vfCashSum.toStringAsFixed(0)} ج.م',
        fontSize: 7.5));
    addRtl(_receiptRowHelper(
        'InstaPay', '${instapaySum.toStringAsFixed(0)} ج.م',
        fontSize: 7.5));
    addRtl(_receiptRowHelper('فيزا', '${visaSum.toStringAsFixed(0)} ج.م',
        fontSize: 7.5));
    ...
    addRtl(_receiptRowHelper(
        'إجمالي الدخل الكلي', '${totalRevenue.toStringAsFixed(0)} ج.م',
        fontSize: 8));
    addRtl(_receiptRowHelper(
        'إجمالي تكلفة قطع الغيار', '${totalPartsCost.toStringAsFixed(0)} ج.م',
        fontSize: 8));
    addRtl(_receiptRowHelper(
        'إجمالي مستحقات الفنيين', '${totalTechEarnings.toStringAsFixed(0)} ج.م',
        fontSize: 8));
    addRtl(_receiptRowHelper(
        'إجمالي صافي المحل', '${totalStoreEarnings.toStringAsFixed(0)} ج.م',
        fontSize: 8.5));
  ...
  static Future<Uint8List> generateSalesReceiptPdf(Sale sale) async {
    ...
    String cleanComplaint = DatabaseHelper.complaintNumber.replaceAll(RegExp(r'[^\d]'), '');
    if (cleanComplaint.startsWith('0')) {
      cleanComplaint = '2$cleanComplaint';
    } else if (!cleanComplaint.startsWith('2')) {
      cleanComplaint = '20$cleanComplaint';
    }
    final waPhone = cleanComplaint;
    pw.MemoryImage? qrImage;
    final String qrData                  pw.Text(_cleanAndShapeText('المنتجات المباعة:'),
                    style: pw.TextStyle(
                        fontSize: 10, fontWeight: pw.FontWeight.bold)),
                pw.SizedBox(height: 4),
                  ...
                  return pw.Padding(
                    ...
                    child: pw.Row(
                      ...
                      children: [
                        pw.Expanded(
                          flex: 3,
                          child: pw.Text(_cleanAndShapeText(name),
            ...
            child: pw.Column(
              ...
              children: [
                ...
                pw.Align(
                  alignment: pw.Alignment.center,
                  child: pw.Text(
                    _cleanAndShapeText('فاتورة بيع منتجات'),
                    style: pw.TextStyle(
                  ...
                  return pw.Padding(
                    ...
                    child: pw.Row(
                      ...
                      children: [
                        ...
                        pw.Expanded(
                          flex: 2,
                          child: pw.Text(price.toStringAsFixed(1),
                              style: const pw.TextStyle(fontSize: 8.5),
                              textAlign: pw.TextAlign.center),
                        ),
                        pw.Expanded(
                          flex: 2,
                          child: pw.Text('${total.toStringAsFixed(1)} ج.م',
                              style: pw.TextStyle(
                                  fontSize: 8.5,
                                  fontWeight: pw.FontWeight.bold),
                              textAlign: pw.TextAlign.left),
                ...
                receiptRow('المجموع الفرعي',
                    '${sale.totalAmount.toStringAsFixed(2)} ج.م'),
                if (sale.discount > 0)
                  receiptRow(
                      'الخصم', '${sale.discount.toStringAsFixed(2)} ج.م'),
                receiptRow('الإجمالي النهائي',
                    '${sale.finalAmount.toStringAsFixed(2)} ج.م'),
                receiptRow('طريقة الدفع',
