...
class DatabaseHelper {
  ...
  static Future<int> _generateNextId(
      DatabaseExecutor db, String tableName) async {
    try {
      final result =
      ...
      if (deletes.isNotEmpty) {
        ...
        for (var del in deletes) {
          ...
          try {
            ...
            await database.delete('deleted_records',
                where: 'id = ?', whereArgs: [del['id']]);
          } catch (e) {
            debugPrint(
  ...
  static Future<void> _syncTable(
      String tableName, String pk, SupabaseClient client) async {
    final database = await db;
    ...
    for (var localRow in localRows) {
      ...
      if (!remoteMap.containsKey(id)) {
        ...
        if (!rowsAreEqual(localRow, remoteRow, columns)) {
          final mergedRow =
              mergeRows(null, localRow, remoteRow, columns, tableName);
    ...
    if (toUpload.isNotEmpty) {
      ...
      for (var i = 0; i < toUpload.length; i += batchSize) {
        final end =
            (i + batchSize < toUpload.length) ? i + batchSize : toUpload.length;
    ...
    for (var col in columns) {
      ...
      if (localChanged && remoteChanged) {
        ...
        if (col == 'status' && tableName == 'tickets') {
          ...
          merged[col] =
              localUpdatedAt >= remoteUpdatedAt ? localVal : remoteVal;
  ...
  static bool rowsAreEqual(Map<String, dynamic> rowA, Map<String, dynamic> rowB,
      List<String> columns) {
    for (var col in columns) {
    ...
    final databaseInstance = await openDatabase(
      ...
      onUpgrade: (db, oldVersion, newVersion) async {
        ...
        if (oldVersion < 8) {
          ...
          try {
            await db
                .execute('ALTER TABLE devices ADD COLUMN category_id INTEGER');
          } catch (_) {}
        ...
        if (oldVersion < 16) {
          try {
            await db
                .execute('ALTER TABLE tickets ADD COLUMN updatedAt INTEGER');
  ...
  static Future<List<Ticket>> loadTickets() async {
    ...
    final List<Map<String, dynamic>> maps =
        await database.query('tickets', orderBy: 'id DESC');
  ...
  static Future<void> saveSpareParts(List<SparePart> parts) async {
    ...
    await database.transaction((txn) async {
      ...
      final batch = txn.batch();
      for (var p in parts) {
        batch.insert('spare_parts', p.toJson(),
            conflictAlgorithm: ConflictAlgorithm.replace);
      }
      await batch.commit(noResult: true);
  ...
  static Future<void> deleteSparePart(int id) async {
    final database = await db;
    final List<Map<String, dynamic>> maps =
        await database.query('spare_parts', where: 'id = ?', whereArgs: [id]);
  ...
  static Future<List<Sale>> loadSales() async {
    final database = await db;
    final List<Map<String, dynamic>> maps =
        await database.query('sales', orderBy: 'id DESC');
  ...
  static Future<List<ReturnTransaction>> loadReturns() async {
    final database = await db;
    final List<Map<String, dynamic>> maps =
        await database.query('returns', orderBy: 'id DESC');
  ...
  static Future<void> addTechnician(String name, String phone,
      {String? mobilePasswordHash, String? email}) async {
    final database = await db;
  ...
  static Future<void> updateTechnician(int id, String name, String phone,
      {String? email, String? mobilePasswordHash}) async {
    final database = await db;
    // Get old data first
    ...
    final oldEmail =
        oldTechs.isNotEmpty ? oldTechs.first['email'] as String? : null;
    final oldName =
        oldTechs.isNotEmpty ? oldTechs.first['name'] as String? : null;
      ...
      try {
        ...
        if (oldEmail != null && oldEmail.trim() != activeEmail) {
          await database.delete('users',
              where: 'email = ?', whereArgs: [oldEmail.trim()]);
        }
  ...
  static Future<void> setTechnicianMobilePassword(
      int id, String passwordHash) async {
    final database = await db;
    await database.update(
      'technicians',
    ...
    if (accessory.id != null) {
      ...
      if (existing.isNotEmpty) {
        ...
        if (oldAcc['quantity'] != accessory.quantity)
          changes
              .add('الكمية من ${oldAcc['quantity']} إلى ${accessory.quantity}');
        ...
        details =
            changes.isEmpty ? 'تعديل البيانات العامة' : changes.join('، ');
      }
    }
  ...
  static Future<List<DeferredPayment>> loadDeferredPayments() async {
    final database = await db;
    final List<Map<String, dynamic>> maps =
        await database.query('deferred_payments', orderBy: 'id DESC');
  ...
  static Future<void> deleteDeferredPayment(int id) async {
    ...
    final List<Map<String, dynamic>> maps = await database
        .query('deferred_payments', where: 'id = ?', whereArgs: [id]);
    String name = 'غير معروف';
    double amount = 0.0;
    if (maps.isNotEmpty) {
    ...
    await database
        .delete('deferred_payments', where: 'id = ?', whereArgs: [id]);
    await database.delete('deferred_payments_history',
        where: 'deferredId = ?', whereArgs: [id]);
  ...
  static Future<int> addDeferredPaymentHistory(
      DeferredPaymentHistory history) async {
    final database = await db;
    final id =
        await _insert(database, 'deferred_payments_history', history.toJson());
  ...
  static Future<List<Supplier>> loadSuppliers() async {
    final database = await db;
    final List<Map<String, dynamic>> maps =
        await database.query('suppliers', orderBy: 'id DESC');
  ...
  static Future<List<SupplierDebt>> loadSupplierDebts() async {
    final database = await db;
    final List<Map<String, dynamic>> maps =
        await database.query('supplier_debts', orderBy: 'id DESC');
  ...
  static Future<void> deleteSupplierDebt(int id) async {
    final database = await db;
    final List<Map<String, dynamic>> maps = await database
        .query('supplier_debts', where: 'id = ?', whereArgs: [id]);
    String name = 'غير معروف';
    double amount = 0.0;
    if (maps.isNotEmpty) {
    ...
    await database.delete('supplier_payments_history',
        where: 'debtId = ?', whereArgs: [id]);
  ...
  static Future<int> addSupplierPaymentHistory(
      SupplierPaymentHistory history) async {
    final database = await db;
    final id =
        await _insert(database, 'supplier_payments_history', history.toJson());
  ...
  static Future<List<GoodsReceipt>> loadGoodsReceipts() async {
    final database = await db;
    final List<Map<String, dynamic>> maps =
        await database.query('goods_receipts', orderBy: 'id DESC');
    return maps.map((map) => GoodsReceipt.fromJson(map)).toList();
  }

  static Future<int> saveGoodsReceipt(GoodsReceipt receipt,
    ...
    if (receipt.itemType == 'spare_part') {
      ...
      if (parts.isNotEmpty) {
        ...
        // Find next id
        final maxIdResult =
            await database.rawQuery('SELECT MAX(id) FROM spare_parts');
  ...
  static Future<List<InventoryTransfer>> loadInventoryTransfers() async {
    final database = await db;
    final List<Map<String, dynamic>> maps =
        await database.query('inventory_transfers', orderBy: 'id DESC');
    return maps.map((map) => InventoryTransfer.fromJson(map)).toList();
  }

  static Future<int> saveInventoryTransfer(
      InventoryTransfer transfer, int sourceItemId) async {
    final database = await db;
    ...
    final id =
        await _insert(database, 'inventory_transfers', transfer.toJson());
    await logModification(
  ...
  static Future<List<InventoryAudit>> loadInventoryAudits() async {
    final database = await db;
    final List<Map<String, dynamic>> maps =
        await database.query('inventory_audits', orderBy: 'id DESC');
  ...
  static Future<List<Warehouse>> loadWarehouses() async {
    final database = await db;
    final List<Map<String, dynamic>> maps =
        await database.query('warehouses', orderBy: 'name ASC');
  ...
  static Future<void> saveIsDarkSetting(bool isDark) async {
    ...
    await database.insert(
      'settings',
      {'key': 'isDark', 'value': isDark ? 'true' : 'false'},
      conflictAlgorithm: ConflictAlgorithm.replace,
    );
    await syncDatabase();
  }
  ...
  static Future<void> saveClientPasswordHash(String hash) async {
    try {
      ...
      await database.insert(
        'settings',
        {'key': 'clientPasswordHash', 'value': hash},
        conflictAlgorithm: ConflictAlgorithm.replace,
      );
      await syncDatabase();
    } catch (e) {
  ...
  static Future<int> saveCategory(Category category) async {
    ...
    await logModification(
      ...
      details:
          'النوع: ${category.type == 'accessory' ? 'إكسسوار' : category.type == 'spare_part' ? 'قطعة غيار' : category.type == 'device_brand' ? 'ماركة جهاز' : 'حالة جهاز'}',
    );
    await syncDatabase();
    return id;
  }
  ...
  static Future<void> saveRegistrationDetails({
    ...
    try {
      ...
      await database.transaction((txn) async {
        ...
        await txn.insert(
          'settings',
          {'key': 'clientPasswordHash', 'value': passwordHash},
          conflictAlgorithm: ConflictAlgorithm.replace,
        );
        ...
        if (mutableValues['id'] == null) {
          final result =
              await txn.rawQuery('SELECT MAX(id) as max_id FROM users');
          int maxId = 0;
          if (result.isNotEmpty && result.first['max_id'] != null) {
            maxId = result.first['max_id'] as int;
          }
  ...
  static Future<List<AppUser>> loadUsers() async {
    try {
      final database = await db;
      final List<Map<String, dynamic>> maps =
          await database.query('users', orderBy: 'id ASC');
  ...
  static Future<Attendance?> checkIn(String userName, String userRole,
      {int? userId, String? notes}) async {
    try {
      final database = await db;
      ...
      final existing = await database.query('attendance',
          where: 'userName = ? AND date = ?', whereArgs: [userName, today]);
      if (existing.isNotEmpty) {
      ...
      if (syncConfig != null && _branchesConfig != null) {
        ...
        if (syncRepoUrl != null && syncRepoUrl.isNotEmpty) {
          ...
          if (matchingBranches.isNotEmpty) {
            ...
            targetBranch = _branchesConfig!.currentBranch ??
          ...
          if (targetBranch != null) {
            ...
            if (hasChanges) {
              ...
              final index = _branchesConfig!.branches
                  .indexWhere((b) => b.id == targetBranch!.id);
              if (index != -1) {
  ...
  static Future<void> deleteBranch(int branchId) async {
    ...
    if (config.currentBranchId == branchId) {
      config.currentBranchId =
          config.branches.isNotEmpty ? config.branches.first.id : 1;
    }
    await _saveBranchesConfig(config);
