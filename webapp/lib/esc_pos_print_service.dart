...
// Uses the WebUSB API via JavaScript bridge to send ESC/POS commands
...
class EscPosPrintService {
  ...
  /// Check if a specific printer type is currently connected via WebUSB.
  ...
  static Future<bool> connectPrinter({required String type}) async {
    ...
    try {
      ...
      if (result['success'] == true) {
        ...
        debugPrint(
          'USB printer connected ($type): vendor=$newVid, product=$newPid, name=$productName',
        );
  ...
  /// Try to auto-reconnect to a previously authorized USB printer (no dialog).
  ...
  /// Send raw ESC/POS data to a connected USB printer.
  static Future<bool> printData(String type, Uint8List data) async {
    if (!kIsWeb || !isConnected(type)) return false;
    try {
      final jsonStr = await jsPrintUsbPrint(type, data);
      final result = jsonDecode(jsonStr) as Map<String, dynamic>;
      return result['success'] == true;
    } catch (e) {
      debugPrint('EscPosPrintService.printData($type) error: $e');
      return false;
    }
  }
  ...
  /// Must be called on app startup BEFORE autoReconnect.
  ...
  static Future<bool> printLabel(Ticket ticket) async {
    ...
    try {
      ...
      return await printData(labelPrinterType, data);
    } catch (e) {
  ...
  static Future<bool> printReceipt(Ticket ticket, {int copies = 1}) async {
    ...
    try {
      for (int i = 0; i < copies; i++) {
        ...
        final ok = await printData(receiptPrinterType, data);
        if (!ok) {
