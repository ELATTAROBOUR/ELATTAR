...
class HwidService {
  ...
  static const String _hwidKey = "ELATTAR_STORE_SECURE_HWID_SECRET_2026";
  // Secret salt for License Key generation
  static const String _salt = "ELATTAR_STORE_SECURE_SALT_KEY_2026";
  // Secret salt for Reset Key generation
  static const String _resetSalt = "ELATTAR_STORE_RESET_SALT_KEY_2026";
  ...
  static Future<String> getRawHardwareDetails() async {
    // Android: Generate a persistent device ID using SharedPreferences + device info
    if (Platform.isAndroid) {
      final prefs = await SharedPreferences.getInstance();
      String? deviceUuid = prefs.getString('device_hwid_uuid');
      if (deviceUuid == null || deviceUuid.isEmpty) {
        final random = Random.secure();
        deviceUuid = List<int>.generate(
          16,
          (_) => random.nextInt(256),
        ).map((b) => b.toRadixString(16).padLeft(2, '0')).join();
        await prefs.setString('device_hwid_uuid', deviceUuid);
      }
      final hostname = Platform.localHostname;
      final osVersion = Platform.operatingSystemVersion;
      return "ANDROID:$deviceUuid|HOST:$hostname|OS:$osVersion";
    }
  ...
  static Future<bool> verifyLicense(String key) async {
    ...
    try {
      ...
      final keyExpiry = parts[0];
      final signatureHex = parts[1];

      final currentHwid = (await getHWID()).replaceAll('-', '').toUpperCase();
      final payload = "$currentHwid|$keyExpiry";
      ...
      final sigToVerify = Signature(sigBytes, publicKey: publicKey);
  ...
  static List<int> _xorCipher(List<int> bytes, String key) {
    ...
    for (int i = 0; i < bytes.length; i++) {
      result[i] =
          bytes[i] ^ keyBytes[i % keyBytes.length] ^ ((i * 47 + 139) % 256);
    }
    return result;
  }
}
